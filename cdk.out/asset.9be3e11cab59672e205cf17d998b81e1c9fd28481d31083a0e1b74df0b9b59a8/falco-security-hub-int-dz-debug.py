...
def ecs_convert_falco_log_to_asff(entry):
    ...
    try:
      instance_id = entry["ec2_instance_id"]
      print("ECS to ASFF got instance_id: ",instance_id)
      instance = get_ec2_details(instance_id)
    except:
      print("ECS to ASFF looks like there's no element for 'ec2_instance_id'")
      #instance = null
    finally:
      print("ECS to ASFF got instance_id: ", instance_id)
    ...
    print ("PROBLEM: ECS to ASFF got account ID, this_id ", account_id, this_id)

    #check whether instance is null or not
    if instance is None:
       print("Could not get instance_resource from NULL instance!")
    else:
...
# Retrieve details of EKS falco log message
def get_eks_details(message):

    log = message["log"]
    print("-------------------------------------------")
    print ("NB! In get_eks_details log structure is: ",log)
...
def eks_convert_falco_log_to_asff(entry):
    ...
    try:
      instance_id = entry["ec2_instance_id"]
      print("EKS to ASFF got instance_id: ",instance_id)
      instance = get_ec2_details(instance_id)
    except:
    ...
    # try to parse the log entry for 'output' elements
    print ("EKS to ASFF got log entry: ", entry["log"] )
    print("-------------------------------------------")
    ...
    try:
      ...
      print("EKS to ASFF looks like there's no output or priority entyries in the Log above!")
...
def lambda_handler(event, context):
    ...
    for entry in data['logEvents']:
        ...
        if "ecs_cluster" in message:
            # Add debug print
            print ("-------------------------------------------------------------")
            print ("BEFORE calling ECS_convert_falco_log_to_asff log entry is: ", message)
            print ("-------------------------------------------------------------")
            # end debug print
            ...
            # Add debug print
            print ("-------------------------------------------------------------")
            print ("BEFORE calling EKS_convert_falco_log_to_asff log entry is: ", message)
            print ("-------------------------------------------------------------")
            # end debug print
